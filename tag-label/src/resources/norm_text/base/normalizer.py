...
from configs.emoji import EMOTICONS
from configs.alphabet_vi import ALPHABET_VI
from configs.path import VIETNAMESE_STOPWORDS, CUSTOMER_NAMES
...
class TextNormalizer():
    ...
    # def remove_stopword(self, input_str):
    #     """
    #     Remove vietnamese stopwords in input_str
    #     """
    #     stopwords = [line.rstrip("\n") for line in open(VIETNAMESE_STOPWORDS)]
    #     sentences_token = annotator.tokenize(input_str)
    #     tokens = [token for sentence_token in sentences_token for token in sentence_token]
    ...
    def normalize_phone_number(self, input_str):
        ...
        p = regex.compile(r"(?<=[h|H]otline|[T|t]ổng đài|[Đ|đ]iện thoại|SDT|SĐT|[z|Z]alo|đường dây nóng|[L|l]iên hệ|[g|G]ọi|[C|c]all|[C|c]hi tiết|[H|h]ỗ trợ|[T|t]ư vấn|[L|l]iên lạc|[C|c]ông ty|[b|B]án [h|H]àng|[đ|Đ]ặt hàng)\s*(.*)(\s*)(\d{8,12}|\d{4}\s\d{2}\s\d{2}\s\d{2}|\d{2,5}\s\d{2,4}\s\d{2,4}\s\d{2,4}|\d{5}\.\d{5}|\d{3,5}\s\d{2,4}\s\d{2,4}|\d{3}\s\d{3}\s\d{4}|\d{3,5}\s\d{5,7}|\d{3}\.\d{4}\.\d{3}|\d{3}\.\d{3}\.\d{4}|\d{4}\.\d{3}\.\d{3}|\d{2}\.\d{4}\.\d{4}|\d{4}\.\d{2}\.\d{4}|\d{4}\.\d{4}\.\d{2}|\d{4}\.\d{4}\.\d{2}|\d{4}\.\d{2}\.\d{2}\.\d{2}|\d{3})([\s|.|,|)])")
        temp_str = input_str
        phone_number_list = []
    ...
    def norm_tag_verbatim(self, input_str):
        ...
        for e in verbatim:
            ...
            if e == '#':
                input_str = input_str.replace(e, ' ' + 'thăng' + ' ')
            elif e == '$':
                input_str = input_str.replace(e, ' ' + 'đô la' + ' ')
            elif e == '%':
                input_str = input_str.replace(e, ' ' + 'phần trăm' + ' ')
            elif e == '&':
                input_str = input_str.replace(e, ' ' + 'và' + ' ')
            # elif e == '*':
            #     input_str = input_str.replace(e, ' ' + 'nhân' + ' ')
            elif e == '+':
                input_str = input_str.replace(e, ' ' + 'cộng' + ' ')
            elif e == '<':
                input_str = input_str.replace(e, ' ' + 'nhỏ hơn' + ' ')
            elif e == '=':
                input_str = input_str.replace(e, ' ' + 'bằng' + ' ')
            elif e == '>':
                input_str = input_str.replace(e, ' ' + 'lớn hơn' + ' ')
            elif e == '@':
                input_str = input_str.replace(e, ' ' + 'a còng' + ' ')
            elif e == '^':
                input_str = input_str.replace(e, ' ' + 'mũ' + ' ')
            elif e == '\\':
                input_str = input_str.replace(e, ' ' + 'trên' + ' ')
            elif e == '~':
                input_str = input_str.replace(e, ' ' + 'tương đương' + ' ')
            ...
            for item in number_letter:
                ...
                for character in characters:
                    ...
                    for i in range(len(character)):
                        temp_character_str += ' ' + ALPHABET_VI[character[i]] + ' '
    ...
    def norm_tag_fraction4(self, input_str):
        ...
        """
        input_str = ' ' + input_str + ' '
        p = re.compile(r"\s[AĂÂÁẮẤÀẰẦẢẲẨÃẴẪẠẶẬĐEÊÉẾÈỀẺỂẼỄẸỆIÍÌỈĨỊOÔƠÓỐỚÒỒỜỎỔỞÕỖỠỌỘỢUƯÚỨÙỪỦỬŨỮỤỰYÝỲỶỸỴAĂÂÁẮẤÀẰẦẢẲẨÃẴẪẠẶẬĐEÊÉẾÈỀẺỂẼỄẸỆIÍÌỈĨỊOÔƠÓỐỚÒỒỜỎỔỞÕỖỠỌỘỢUƯÚỨÙỪỦỬŨỮỤỰYÝỲỶỸỴAĂÂÁẮẤÀẰẦẢẲẨÃẴẪẠẶẬĐEÊÉẾÈỀẺỂẼỄẸỆIÍÌỈĨỊOÔƠÓỐỚÒỒỜỎỔỞÕỖỠỌỘỢUƯÚỨÙỪỦỬŨỮỤỰYÝỲỶỸỴAĂÂÁẮẤÀẰẦẢẲẨÃẴẪẠẶẬĐEÊÉẾÈỀẺỂẼỄẸỆIÍÌỈĨỊOÔƠÓỐỚÒỒỜỎỔỞÕỖỠỌỘỢUƯÚỨÙỪỦỬŨỮỤỰYÝỲỶỸỴAĂÂÁẮẤÀẰẦẢẲẨÃẴẪẠẶẬĐEÊÉẾÈỀẺỂẼỄẸỆIÍÌỈĨỊOÔƠÓỐỚÒỒỜỎỔỞÕỖỠỌỘỢUƯÚỨÙỪỦỬŨỮỤỰYÝỲỶỸỴAĂÂÁẮẤÀẰẦẢẲẨÃẴẪẠẶẬĐEÊÉẾÈỀẺỂẼỄẸỆIÍÌỈĨỊOÔƠÓỐỚÒỒỜỎỔỞÕỖỠỌỘỢUƯÚỨÙỪỦỬŨỮỤỰYÝỲỶỸỴA-Zaăâáắấàằầảẳẩãẵẫạặậđeêéếèềẻểẽễẹệiíìỉĩịoôơóốớòồờỏổởõỗỡọộợuưúứùừủửũữụựyýỳỷỹỵaăâáắấàằầảẳẩãẵẫạặậđeêéếèềẻểẽễẹệiíìỉĩịoôơóốớòồờỏổởõỗỡọộợuưúứùừủửũữụựyýỳỷỹỵaăâáắấàằầảẳẩãẵẫạặậđeêéếèềẻểẽễẹệiíìỉĩịoôơóốớòồờỏổởõỗỡọộợuưúứùừủửũữụựyýỳỷỹỵaăâáắấàằầảẳẩãẵẫạặậđeêéếèềẻểẽễẹệiíìỉĩịoôơóốớòồờỏổởõỗỡọộợuưúứùừủửũữụựyýỳỷỹỵaăâáắấàằầảẳẩãẵẫạặậđeêéếèềẻểẽễẹệiíìỉĩịoôơóốớòồờỏổởõỗỡọộợuưúứùừủửũữụựyýỳỷỹỵaăâáắấàằầảẳẩãẵẫạặậđeêéếèềẻểẽễẹệiíìỉĩịoôơóốớòồờỏổởõỗỡọộợuưúứùừủửũữụựyýỳỷỹỵa-z]+\/")
        temp_str = input_str
        ratio_list = []
        while(p.search(temp_str)):
            ratio = p.search(temp_str)
    ...
    def norm_tag_measure(self, input_str):
        ...
        mp_pattern = '\s[0-9]*\.*\,*\-*[0-9]+\s*MP[\s|.|,|)]'	
        byte_pattern = '\s[0-9]*\.*\,*\-*[0-9]+\s*byte[\s|.|,|)]'
        kilobyte_pattern = '\s[0-9]*\.*\,*\-*[0-9]+\s*KB[\s|.|,|)]'
        megabyte_pattern = '\s[0-9]*\.*\,*\-*[0-9]+\s*MB[\s|.|,|)]'
        Mb_pattern = '\s[0-9]*\.*\,*\-*[0-9]+\s*Mb[\s|.|,|)]'
        mb_pattern = '\s[0-9]*\.*\,*\-*[0-9]+\s*mb[\s|.|,|)]'
        Gb_pattern = '\s[0-9]*\.*\,*\-*[0-9]+\s*Gb[\s|.|,|)]'
        GB_pattern = '\s[0-9]*\.*\,*\-*[0-9]+\s*GB[\s|.|,|)]'
        gb_pattern = '\s[0-9]*\.*\,*\-*[0-9]+\s*gb[\s|.|,|)]'
        terabyte_pattern = '\s[0-9]*\.*\,*\-*[0-9]+\s*TB[\s|.|,|)]'
        Tb_pattern = '\s[0-9]*\.*\,*\-*[0-9]+\s*Tb[\s|.|,|)]'
        ...
        l_pattern = '\s[0-9]*\.*\,*\-*[0-9]+\s*l[\s|.|,|)]'
        ml_pattern = '\s[0-9]*\.*\,*\-*[0-9]+\s*ml[\s|.|,|)]'
        kg_pattern = '\s[0-9]*\.*\,*\-*[0-9]+\s*kg[\s|.|,|)]'
        Kg_pattern = '\s[0-9]*\.*\,*\-*[0-9]+\s*Kg[\s|.|,|)]'
        g_pattern = '\s[0-9]*\.*\,*\-*[0-9]+\s*g[\s|.|,|)]'
        gram_pattern = '\s[0-9]*\.*\,*\-*[0-9]+\s*gram[\s|.|,|)]'
        gr_pattern = '\s[0-9]*\.*\,*\-*[0-9]+\s*gr[\s|.|,|)]'
        mg_pattern = '\s[0-9]*\.*\,*\-*[0-9]+\s*mg[\s|.|,|)]'
        wat_pattern = '\s[0-9]*\.*\,*\-*[0-9]+\s*W[\s|.|,|)]'
        kwat_pattern = '\s[0-9]*\.*\,*\-*[0-9]+\s*kW[\s|.|,|)]'
        VND_pattern = '\s[0-9]*\.*\,*\-*[0-9]+\s*VND[\s|.|,|)]'
        # VND_pattern = '\s[0-9]*\.*\,*\-*[0-9]+\s*[VNĐ[\s|.|,|)]'
        vnd_pattern = '\s[0-9]*\.*\,*\-*[0-9]+\s*vnd[\s|.|,|)]'
        đ_pattern = '\s[0-9]*\.*\,*\-*[0-9]+\s*đ[\s|.|,|)]'
        # VND_pattern = '\s[0-9]*\.*\,*\-*[0-9]+\s*[vnđ[\s|.|,|)]'
        money_pattern = '\s[0-9]*\.*\,*\-*[0-9]+\s*K[\s|.|,|)]'
        money1_pattern = '\s[0-9]*\.*\,*\-*[0-9]+\s*k[\s|.|,|)]'
        tr_pattern = '\s[0-9]*\.*\,*\-*[0-9]+\s*tr[\s|.|,|)]'
        usd_pattern = '\s[0-9]*\.*\,*\-*[0-9]+\s*usd[\s|.|,|)]'
        euro_pattern = '\s[0-9]*\.*\,*\-*[0-9]+\s*eur[\s|.|,|)]'
        Euro_pattern = '\s[0-9]*\.*\,*\-*[0-9]+\s*Eur[\s|.|,|)]'		
        # đ_pattern = '\s[0-9]*\.*\,*\-*[0-9]+\s*đ(\s|.)'

        input_str = self.norm_tag_measure_generic(input_str, mp_pattern, 'MP', ' mê ga píc seo ')
        input_str = self.norm_tag_measure_generic(input_str, byte_pattern, 'byte', ' bai ')
        input_str = self.norm_tag_measure_generic(input_str, kilobyte_pattern, 'KB', ' ki lô bai ')
        input_str = self.norm_tag_measure_generic(input_str, megabyte_pattern, 'MB', ' mê ga bai ')
        input_str = self.norm_tag_measure_generic(input_str, Mb_pattern, 'Mb', ' mê ga bai ')
        input_str = self.norm_tag_measure_generic(input_str, mb_pattern, 'mb', ' mê ga bai ')
        input_str = self.norm_tag_measure_generic(input_str, Gb_pattern, 'Gb', ' ghi ga bai ')
        input_str = self.norm_tag_measure_generic(input_str, GB_pattern, 'GB', ' ghi ga bai ')
        input_str = self.norm_tag_measure_generic(input_str, gb_pattern, 'gb', ' ghi ga bai ')
        input_str = self.norm_tag_measure_generic(input_str, terabyte_pattern, 'TB', ' tê ra bai ')
        input_str = self.norm_tag_measure_generic(input_str, Tb_pattern, 'Tb', ' tê ra bai ')
        input_str = self.norm_tag_measure_generic(input_str, km_pattern, 'km', ' ki lô mét ')
        input_str = self.norm_tag_measure_generic(input_str, m_pattern, 'm', ' mét ')
        input_str = self.norm_tag_measure_generic(input_str, cm_pattern, 'cm', ' xen ti mét ')
        input_str = self.norm_tag_measure_generic(input_str, mm_pattern, 'mm', ' mi li mét ')
        input_str = self.norm_tag_measure_generic(input_str, nm_pattern, 'nm', ' na nô mét ')
        input_str = self.norm_tag_measure_generic(input_str, ha_pattern, 'ha', ' héc ta ')
        input_str = self.norm_tag_measure_generic(input_str, L_pattern, 'L', ' lít ')
        input_str = self.norm_tag_measure_generic(input_str, l_pattern, 'l', ' lít ')
        input_str = self.norm_tag_measure_generic(input_str, ml_pattern, 'ml', ' mi li lít ')
        input_str = self.norm_tag_measure_generic(input_str, kg_pattern, 'kg', ' ki lô gam ')
        input_str = self.norm_tag_measure_generic(input_str, Kg_pattern, 'Kg', ' ki lô gam ')
        input_str = self.norm_tag_measure_generic(input_str, g_pattern, 'g', ' gam ')
        input_str = self.norm_tag_measure_generic(input_str, gram_pattern, 'gram', ' gờ ram ')
        input_str = self.norm_tag_measure_generic(input_str, gr_pattern, 'gr', ' gờ ram ')
        input_str = self.norm_tag_measure_generic(input_str, mg_pattern, 'mg', ' mi li gam ')
        input_str = self.norm_tag_measure_generic(input_str, wat_pattern, 'W', ' goát ')
        input_str = self.norm_tag_measure_generic(input_str, kwat_pattern, 'kW', ' ki lô goát ')
        input_str = self.norm_tag_measure_generic(input_str, VND_pattern, 'VND', ' việt nam đồng ')
        input_str = self.norm_tag_measure_generic(input_str, vnd_pattern, 'vnd', ' việt nam đồng ')
        input_str = self.norm_tag_measure_generic(input_str, đ_pattern, 'đ', ' đồng ')
        input_str = self.norm_tag_measure_generic(input_str, money_pattern, 'K', ' nghìn ')
        input_str = self.norm_tag_measure_generic(input_str, money1_pattern, 'k', ' nghìn ')
        input_str = self.norm_tag_measure_generic(input_str, tr_pattern, 'tr', ' triệu ')
        input_str = self.norm_tag_measure_generic(input_str, usd_pattern, 'usd', ' u ét đê ')
        input_str = self.norm_tag_measure_generic(input_str, euro_pattern, 'eur', ' ơ rô ')
        input_str = self.norm_tag_measure_generic(input_str, Euro_pattern, 'Eur', ' ơ rô ')
        # input_str = self.norm_tag_measure_generic(input_str, đ_pattern, 'đ ', ' đồng ')
        # print(input_str)
        return input_str
    ...
    def norm_tag_measure_generic(self, input_str, pattern, term, norm_term):
        ...
        if len(matches) > 0:
            for item in matches:
                ...
                input_str = input_str.replace(item, ' ' + item_norm_out.strip() + ' ')
        return input_str
    ...
    def norm_account_name(self, input_str):
        """
            Normalize account name in comment
            """
        input_str = ' ' + input_str + ' '
        # p = re.compile(r"[\d ]{9,20}")
        p = re.compile(r"([T|t]hân [G|g]ửi|[C|c]hào [B|b]ạn|[C|c]ảm ơn [B|b]ạn|[C|c]ảm ơn|[C|c]hào|[A|a]\/[C|c]hị|[C|c]ám ơn|[A|a]nh]\/[C|c]hị)+\s*([a-zA-Z]{1,20}\**\d{1,20}[a-zA-Z]{1,20}|\d{1,20}[*]*[a-zA-Z]{1,20}\d{1,20}|[a-zA-Z]{1,20}[*]*\d{1,20}|\d{1,20}[*]*[a-zA-Z]{1,20}|[a-zA-Z]{1,20}[*]+[a-zA-Z]{1,20})")  
        temp_str = input_str
        account_list = []
        while(p.search(temp_str)):
            account = p.search(temp_str)
            term = account.group()
            x = re.search(r"([a-zA-Z]{1,20}\**\d{1,20}[a-zA-Z]{1,20}|\d{1,20}[*]*[a-zA-Z]{1,20}\d{1,20}|[a-zA-Z]{1,20}[*]*\d{1,20}|\d{1,20}[*]*[a-zA-Z]{1,20}|[a-zA-Z]{1,20}[*]+[a-zA-Z]{1,20})",term)
            account_list.append(x.group())
            temp_str = temp_str[account.span()[1]-1:]
        # digits = re.findall(r"[\d ]{9,20}", input_str)

        if len(account_list) > 0:
            for account in account_list:
                customer_name = [line.rstrip("\n") for line in open(CUSTOMER_NAMES)]
                replace_name = random.choice(customer_name)
                input_str = input_str.replace(account, ' ' + replace_name + ' ')
    ...
    def norm_math_characters(self, input_str):
        ...
        """
        input_str = ' ' + input_str + ' '
        input_str = math_characters(input_str)
        # p = re.compile(r"[\d ]{9,20}")
        # p = re.compile(r"([T|t]hân [G|g]ửi|[C|c]hào [B|b]ạn|[C|c]ảm ơn [B|b]ạn|[C|c]ảm ơn|[C|c]hào|[A|a]\/[C|c]hị|[C|c]ám ơn|[A|a]nh]\/[C|c]hị)+\s*([a-zA-Z]{1,20}\**\d{1,20}[a-zA-Z]{1,20}|\d{1,20}[*]*[a-zA-Z]{1,20}\d{1,20}|[a-zA-Z]{1,20}[*]*\d{1,20}|\d{1,20}[*]*[a-zA-Z]{1,20}|[a-zA-Z]{1,20}[*]+[a-zA-Z]{1,20})")  
        # temp_str = input_str
        # account_list = []
    ...
    def norm_duplicate_word(self, input_str):
        ...
        # input_str = remove_adjacent_word(input_str, "?")
        # input_str = remove_redundant_words(input_str, 'mùng', 'ngày')
        return input_str